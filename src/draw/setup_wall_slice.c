...
{
	if (game->ray.side == 0)
		slice->wall_hit = game->player.y + game->ray.perp_wall_dist
			* game->ray.dir_y;
	else
		slice->wall_hit = game->player.x + game->ray.perp_wall_dist
			* game->ray.dir_x;
	slice->wall_hit = 1.0 - (slice->wall_hit - floor(slice->wall_hit));
	slice->tex_x = (int)(slice->wall_hit * (double)slice->tex->img_width);
